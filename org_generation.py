    ...
    def submitOrder(self, url, user = 'admin', password = 'welcome1'):
        ...
        try:
            ...
            if success_response_reference and success_order_id and success_order_type and success_version_number:
                ...
                print(f"\nCartridge Version Number: {success_version_number[0].text}")
...
def options():
    ...
    # TODO MAKE ALL OF THESE OPTIONS INTO SEPERATE FUNCTIONS
    ...
    while pick != 0:
        ...
        if pick == 1:
            ...
            if pick == 1:
                choices = ["Provide", "Change-Owner", "Cease", "Move (All)", "Move (Select)"]
                menu(choices)
                    ...
                    if ans == 1:
                        ...
                        try: 
                            ...
                            for i in range(numOrders):
                                order: createOrder = submitRun(credentials)
                                reference_numbers.append(order.reference_number)
                                if i != numOrders - 1:
                                    if batches != None:
                                        if batches == 0:
